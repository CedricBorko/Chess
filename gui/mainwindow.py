...
from PySide6.QtGui import (QBrush, QColor, QFont, QIcon, QKeyEvent, QMouseEvent, QPainter, QPainterPath,
                           QPaintEvent, QPen,
                           QResizeEvent)
from PySide6.QtWidgets import (QButtonGroup, QDialog, QGridLayout, QGroupBox, QLabel, QMainWindow, QPushButton,
                               QScrollArea,
                               QSizePolicy,
                               QVBoxLayout, QWidget)
...
from chess.io.io import load_piece_graphics
...
class Window(QMainWindow):
    def __init__(self):
        ...
        self.setStyleSheet(
            "QWidget{background: #1e1e1e; border: none}"
...
class BoardView(QWidget):
    ...
    def draw_current_move(self, painter: QPainter) -> None:
        if self.move_index not in range(len(self.move_buttons)): return

        painter.setPen(QPen(COLOR_HIGHLIGHT, 10))
        selected_move = self.board.moves[self.move_index]

        painter.drawLine(
            self.OFFSET + selected_move.origin % 8 * self.TILE_SIZE + self.TILE_SIZE // 2,
            self.OFFSET + selected_move.origin // 8 * self.TILE_SIZE + self.TILE_SIZE // 2,
            self.OFFSET + selected_move.target % 8 * self.TILE_SIZE + self.TILE_SIZE // 2,
            self.OFFSET + selected_move.target // 8 * self.TILE_SIZE + self.TILE_SIZE // 2
        )
        painter.setBrush(QBrush(COLOR_HIGHLIGHT))
    ...
    def draw_pieces(self, painter: QPainter) -> None:
        ...
        for i in range(64):
            ...
            if piece is not None:
                painter.drawImage(
                    rect,
                    self.piece_graphics[piece.abbreviation]
                )
    ...
    def paintEvent(self, event: QPaintEvent):
        ...
        if self.winner is not None:
            if self.check_mate:
                painter.drawText(
                    QRect(self.OFFSET, 0, self.TILE_SIZE * 8, self.OFFSET), Qt.AlignmentFlag.AlignCenter,
    ...
    def get_ring_path(self, move):
        ...
        path.addEllipse(
            ...
            self.TILE_SIZE // 2 - 8, self.TILE_SIZE // 2 - 8
        )
        path.addEllipse(
            QPoint(
                self.OFFSET + self.TILE_SIZE // 2 + move.target % 8 * self.TILE_SIZE,
            ...
            self.TILE_SIZE // 4 + 8, self.TILE_SIZE // 4 + 8
        )
        return path
    ...
    def update_legal_moves(self):
        ...
        if self.selected_piece.alliance == self.board.active_player:
            ...
            for move in self.selected_piece.legal_moves:
                ...
                if not copied_board.active_player.is_check(copied_board):
            ...
            if isinstance(self.selected_piece, King):
                for move in updated_legal_moves:
                    if isinstance(move, CastleMove) and self.board.active_player.is_check(self.board):
    ...
    def resizeEvent(self, event: QResizeEvent):
        ...
        self.info_widget.setGeometry(
            self.OFFSET * 2 + 8 * self.TILE_SIZE, self.OFFSET,
            self.width() - 8 * self.TILE_SIZE - self.OFFSET * 3,
            self.TILE_SIZE * 8
        )

        self.promotion_view.setGeometry(
            self.OFFSET + self.TILE_SIZE * 2, self.OFFSET + self.TILE_SIZE * 2,
    ...
    def add_move_button(self, move = None):
        ...
        if move is not None:
            move_button = MoveButton(
                self, len(self.board.moves_done),
